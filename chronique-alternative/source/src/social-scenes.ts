...
export const SOCIAL_SCENES: SocialScene[] = [
  {
    id: "amanea-family-truth", title: "Deux filles et une frontière", characters: ["amanea", "iriana", "naiah"], triggerCharacters: ["amanea"], requiredPresent: ["amanea"], minStages: { amanea: 2 }, excludesFlags: ["amanea-family-truth"], oneTime: true, priority: 110, mood: "sad",
    prompt: [
      line("Narration", "Dans la salle de guerre d’Akuhn’Nabad, Allenna vient de quitter le siège placé à la droite d’Amanea. Un miroir montre Iriana depuis Al’Gratal ; une fleur de brume déposée par Naïah demeure au centre de la table."),
      line("Iriana", "Tu prépares Allenna à gouverner, mais tu refuses toujours d’expliquer pourquoi Naïah doit être chassée. Même moi, je reconnais là une contradiction politique difficile à défendre."),
      line("Naïah", "Je ne lui demande pas de défendre la décision. Je lui demande de me regarder lorsqu’elle admet que c’est la sienne."),
      line("Amanea", "Allenna est mon héritière parce qu’elle sait protéger Akuhn’Nabad sans devenir mon prolongement. Naïah est ma fille même lorsqu’elle me hait. Et tant que le pacte d’Alamma peut utiliser son existence comme une porte, je ne lui ouvrirai pas la cité."),
      line("Narration", "Amanea n’en dit pas davantage. Pour la première fois, cependant, son secret possède une limite identifiable : la chasse prendra fin si la menace du pacte disparaît."),
    ],
    choices: [
      choice("aft-l", "Distinguer l’amour maternel d’Amanea du droit de décider seule pour ses filles.", "lucidite", [line("{player}", "Aimer Naïah n’efface pas ce que la chasse lui fait. Être fière d’Allenna ne vous donne pas le droit d’écrire son règne."), line("Amanea", "Deux vérités désagréables dans une seule phrase."), line("Naïah", "Je commence à apprécier cette personne."), line("Narration", "Amanea accepte qu’Allenna participe désormais aux décisions concernant le pacte.")], { trust: 7, flags: ["amanea-family-truth", "story-family-boundary"], relationshipEffects: { iriana: { trust: 5 }, naiah: { trust: 5 } } }),
      choice("aft-s", "Obtenir un engagement concret sans exiger la révélation immédiate du secret.", "sangFroid", [line("Amanea", "Naïah recevra chaque information qui ne renforcera pas la prise du démon. Allenna vérifiera elle-même ce que je retiens."), line("Naïah", "Je ne te pardonne pas."), line("Amanea", "Je ne te l’ai pas demandé."), line("Narration", "La frontière demeure, mais elle cesse d’être entièrement contrôlée par une seule personne.")], { trust: 8, flags: ["amanea-family-truth", "story-family-boundary"], relationshipEffects: { iriana: { trust: 5 }, naiah: { trust: 6 } } }),
      choice("aft-a", "Demander à Amanea si elle accepterait qu’Allenna désobéisse pour protéger Naïah.", "audace", [line("Amanea", "Elle le ferait probablement avec un plan agaçant et remarquablement efficace."), line("Iriana", "Ce n’était pas la question."), line("Amanea", "Non. La réponse est oui. Si elle juge ma décision injuste, je veux qu’elle possède la force de me l’opposer."), line("Naïah", "Alors elle est peut-être réellement ton héritière.")], { affection: 7, trust: 5, flags: ["amanea-family-truth", "story-family-boundary"], relationshipEffects: { iriana: { affection: 4, trust: 3 }, naiah: { affection: 5, trust: 3 } } }),
    ],
  },
  {
  ...
  {
    id: "medig-window", title: "La messagère blanche", characters: ["hylee", "remerii"], requiredPresent: ["hylee", "remerii"], sublocations: ["miraldas-quarters"], excludesFlags: ["medig-window"], oneTime: true, priority: 95, mood: "soft",
    prompt: [
      ...
      line("Hylee", "Elle t’a trouvé·e aussi. Et elle vient de tracer la route qu’aucune équipe n’a prise. Je crois que Medig nous demande d’enquêter sans attirer l’Empire."),
    ],
    choices: [
  ...
  {
    id: "hr-hylee-truth", title: "Ce qui existe déjà", characters: ["hylee"], triggerCharacters: ["hylee"], requiredPresent: ["hylee"], sublocations: ["miraldas-quarters"], minStages: { hylee: 2 }, excludesFlags: ["hr-started"], oneTime: true, priority: 100, mood: "determined",
    prompt: [
  ...
  {
    id: "hr-remerii-truth", title: "Une vérité sans détour", characters: ["remerii"], triggerCharacters: ["remerii"], requiredPresent: ["remerii"], sublocations: ["miraldas-quarters"], minStages: { remerii: 2 }, excludesFlags: ["hr-started"], oneTime: true, priority: 100, mood: "strict",
    prompt: [
  ...
  {
    id: "hr-secret-remerii", title: "Une discipline impossible", characters: ["remerii"], triggerCharacters: ["remerii"], requiredPresent: ["remerii"], sublocations: ["miraldas-quarters"], minStages: { remerii: 3 }, requiresFlags: ["hr-path-secret", "hr-lover-remerii"], excludesFlags: ["hr-secret-confirmed", "hr-path-honest", "hr-resolution-couple"], oneTime: true, priority: 95, mood: "sad",
    prompt: [
  ...
  {
    id: "hr-triad-morning", title: "Le matin à trois", characters: ["hylee", "remerii"], requiredPresent: ["hylee", "remerii"], sublocations: ["miraldas-quarters"], minStages: { hylee: 4, remerii: 4 }, requiresFlags: ["hr-resolution-triad"], excludesFlags: ["hr-triad-established"], oneTime: true, priority: 80, mood: "soft",
    prompt: [
  ...
  {
    id: "iv-open-truth", title: "Une union qui ne ferme aucune porte", characters: ["iriana", "valurn"], requiredPresent: ["iriana", "valurn"], sublocations: ["algratal-palace-council"], stageSum: 3, excludesFlags: ["iv-informed"], oneTime: true, priority: 85, mood: "calm",
    prompt: [
  ...
  {
    id: "iv-boundaries", title: "Les règles que personne ne devine", characters: ["iriana", "valurn"], requiredPresent: ["iriana", "valurn"], sublocations: ["algratal-palace-council"], minStages: { iriana: 3, valurn: 3 }, requiresAnyFlags: ["iv-open-one", "iv-open-both"], excludesFlags: ["iv-boundaries-set"], oneTime: true, priority: 75, mood: "smirk",
    prompt: [
  ...
  {
    id: "shared-hylee-remerii-atelier", title: "Une leçon à deux voix", characters: ["hylee", "remerii"], locations: ["miraldas"], sublocations: ["miraldas-atelier"], oneTime: false, mood: "teasing",
  ...
  {
    id: "shared-hylee-naiah-visit", title: "La visite rare", characters: ["hylee", "naiah"], locations: ["forbidden"], sublocations: ["forbidden-sanctuary"], oneTime: false, mood: "soft",
  ...
  {
    id: "shared-remerii-saidin", title: "Deux archimages, un paradoxe", characters: ["remerii", "saidin"], locations: ["miraldas"], sublocations: ["miraldas-archives"], oneTime: false, mood: "calm",
  ...
  {
    id: "shared-iriana-lineva", title: "Deux villes sur la même carte", characters: ["iriana", "lineva"], locations: ["forthaven"], sublocations: ["forthaven-war-room"], oneTime: false, mood: "stern",
  ...
  {
    id: "shared-valurn-naiah", title: "Une partie sans pion", characters: ["valurn", "naiah"], locations: ["forbidden"], sublocations: ["forbidden-crossroads", "forbidden-sanctuary"], oneTime: false, mood: "charming",
  ...
  {
    id: "shared-valurn-bellirith", title: "Les enfants du Chaos", characters: ["valurn", "bellirith"], locations: ["akuhn", "algratal"], sublocations: ["akuhn-music-room", "algratal-palace-council"], oneTime: false, mood: "charming",
    prompt: [line("Bellirith", "Mon frère affirme qu’il ne regrette rien. C’est pratique : il n’a ainsi jamais besoin de présenter d’excuses."), line("Valurn", "Ma sœur appelle ‘conversation’ tout échange dont elle connaît déjà le prix."), line("Bellirith", "Nous nous adorons."), line("Valurn", "Nous survivons très élégamment à notre famille. Ce n’est pas pareil."), line("Narration", "Leur cruauté familière dissimule une loyauté qu’aucun des deux ne sait nommer sans perdre la partie.")],
    choices: [
      ...
      choice("svb-truth", "Leur demander ce qu’ils protégeraient chez l’autre.", "lucidite", [line("Valurn", "Son droit de n’appartenir à personne."), line("Bellirith", "Sa capacité agaçante à rester lui-même quand notre père voudrait en faire une arme."), line("Narration", "Ils détournent les yeux exactement au même moment.")], { trust: 3, relationshipEffects: { bellirith: { trust: 4, affection: 1 } } }),
      choice("svb-toast", "Lever un verre à leur famille dysfonctionnelle.", "audace", [line("Valurn", "À ceux qui nous ont ratés."), line("Bellirith", "Et à nous, qui avons eu l’impolitesse de devenir intéressants.")], { affection: 3, relationshipEffects: { bellirith: { affection: 3 } } }),
    ],
  },
  {
    id: "shared-iriana-valurn", title: "Complices sans propriété", characters: ["iriana", "valurn"], locations: ["algratal"], sublocations: ["algratal-palace-council"], oneTime: false, mood: "smirk",
  ...
  {
    id: "shared-bellirith-naiah", title: "Deux reines sans cour", characters: ["bellirith", "naiah"], locations: ["forbidden"], sublocations: ["forbidden-sanctuary"], oneTime: false, mood: "seductive",
